...
module.exports = function packItHelper (context, options) {
    ...
    var packed = _.reduce(context, function (memo, content, i) {
        // First pass of memo is the content object
        // and needs to be run through options.fn

        // i is one ahead at this point so to get
        // the correct packing size we subtract by 1
        memo = typeof memo !== 'string' ?
          options.fn({
            'content': memo,
            'size': packingStructure[i - 1]
          }) :
          memo;
